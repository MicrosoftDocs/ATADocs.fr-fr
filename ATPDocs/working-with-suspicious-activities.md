...
Cet article explique les bases de l’utilisation d’Azure Advanced Threat Protection.

## Examiner les alertes de sécurité dans la chronologie des attaques <a name="review-suspicious-activities-on-the-attack-time-line"></a>
Une fois connecté au portail Azure ATP, vous êtes automatiquement dirigé vers la **chronologie des alertes de sécurité**. Les alertes de sécurité sont listées par ordre chronologique, avec les plus récentes en haut de la liste.
...
-   La capacité à :
    ...
    -   Exporter l’alerte de sécurité dans Excel.
...
-   **Ouvertes** : toutes les nouvelles alertes de sécurité s’affichent dans la liste.

-   **Fermées** : utilisé pour effectuer le suivi des activités que vous avez identifiées, examinées et résolues.

    > [!NOTE]
    > Si une activité est détectée à nouveau après une courte période, Azure ATP peut rouvrir une alerte ayant été fermée.

-   **Ignorées** : permet d’ignorer temporairement une alerte, et de n’être averti de nouveau qu’en cas de nouvelle instance. Cela signifie que si une alerte similaire survient, Azure ATP ne la rouvre pas. Cependant, si l’alerte cesse pendant sept jours puis réapparaît, vous êtes averti à nouveau.
