...
Les alertes de sécurité suivantes vous aident à identifier et à résoudre les activités suspectes de la phase **Dominance du domaine** détectées par [!INCLUDE [Product short](includes/product-short.md)] sur votre réseau. Dans ce tutoriel, vous allez apprendre à comprendre, classifier, prévenir et empêcher les attaques suivantes :
...
> - Suspicion d’utilisation de golden ticket (anomalie de ticket avec la délégation RBCD) (ID externe 2040)
...
Cette détection déclenche une alerte [!INCLUDE [Product short](includes/product-short.md)] quand DPAPI est utilisé pour récupérer la clé principale de sauvegarde.
...
1. La clé privée volée n’est jamais modifiée. Ce qui signifie que l’acteur peut toujours utiliser la clé volée pour déchiffrer les données protégées dans le domaine cible. Il n’existe pas de moyen méthodologique pour changer cette clé privée.
...
1. Incluez les contrôleurs de domaine en procédant ainsi :
...
> [!NOTE]
> Les alertes de tentative d’exécution de code à distance lors d’une tentative d’utilisation des commandes Powershell sont prises uniquement en charge par les capteurs [!INCLUDE [Product short](includes/product-short.md)].
...
Dans cette détection [!INCLUDE [Product short](includes/product-short.md)], une alerte de sécurité est déclenchée quand un ordinateur du réseau tente de s’enregistrer comme contrôleur de domaine non autorisé.
...
> [!NOTE]
> Les alertes de promotion suspecte de contrôleur de domaine (attaque DCShadow potentielle) sont prises en charge par les capteurs [!INCLUDE [Product short](includes/product-short.md)] uniquement.
...
Dans cette détection, une alerte est déclenchée quand une demande de réplication suspecte est générée par rapport à un contrôleur de domaine authentique protégé par [!INCLUDE [Product short](includes/product-short.md)]. Le comportement est révélateur de certaines techniques utilisées dans les attaques DCShadow.
...
> [!NOTE]
> Les alertes de demande de réplication suspecte (attaque DCShadow potentielle) sont prises en charge par les capteurs [!INCLUDE [Product short](includes/product-short.md)] uniquement.
...
> [!NOTE]
> Si vous avez des contrôleurs de domaine sur lesquels les capteurs [!INCLUDE [Product short](includes/product-short.md)] ne sont pas installés, ces contrôleurs de domaine ne sont pas couverts par [!INCLUDE [Product short](includes/product-short.md)]. Si vous déployez un nouveau contrôleur de domaine sur un contrôleur de domaine non inscrit ou non protégé, il peut ne pas être identifié par [!INCLUDE [Product short](includes/product-short.md)] comme contrôleur de domaine tout de suite. Il est vivement recommandé d’installer le capteur [!INCLUDE [Product short](includes/product-short.md)] sur chaque contrôleur de domaine pour obtenir une couverture complète.
...
1. Les utilisateurs sources partagent-ils tous quelque chose en commun ?
   ...
   1. Vérifiez les ressources auxquelles ces tickets ont accédé.
...
1. Incluez l’ordinateur source.
    ...
    - Si vous avez installé Microsoft Defender pour point de terminaison, utilisez **klist.exe purge** pour supprimer tous les tickets de la session spécifiée et empêcher toute utilisation ultérieure des tickets.
...
1. Vérifiez si les ordinateurs qui ont fait l’objet d’un accès ont été corrigés, comme décrit ci-dessus.
...
## <a name="suspected-golden-ticket-usage-ticket-anomaly-using-rbcd-external-id-2040"></a>Suspicion d’utilisation de golden ticket (anomalie de ticket avec la délégation RBCD) (ID externe 2040)
...
1. Le capteur autonome [!INCLUDE [Product short](includes/product-short.md)] impliqué dans cette alerte est-il une machine virtuelle ?
    - Si le capteur autonome [!INCLUDE [Product short](includes/product-short.md)] est impliqué, a-t-il été récemment repris à partir d’un état enregistré ?
...
Le passage à une version inférieure du chiffrement est une méthode visant à affaiblir Kerberos en abaissant le niveau de chiffrement de différents champs du protocole qui ont normalement le niveau de chiffrement le plus élevé. Un champ au chiffrement affaibli peut être plus vulnérable à des attaques de force brute en mode hors connexion. Plusieurs méthodes d’attaque exploitent les codes faibles de chiffrement Kerberos. Dans cette détection, [!INCLUDE [Product short](includes/product-short.md)] apprend les types de chiffrement Kerberos utilisés par les ordinateurs et les utilisateurs. L’alerte est émise quand un chiffrement plus faible inhabituel est utilisé pour l’ordinateur source et/ou l’utilisateur et correspond à des techniques d’attaque connus.
...
1. Examinez le [contrôleur de domaine](investigate-a-computer.md).
1. Vérifiez si Skeleton Key a affecté vos contrôleurs de domaine avec le [scanner écrit par l’équipe [!INCLUDE [Product short](includes/product-short.md)]](https://gallery.technet.microsoft.com/Aorato-Skeleton-Key-24e46b73).
...
1. Incluez le contrôleur de domaine.
...
1. Examinez l’utilisateur source.
...
1. Recherchez les ordinateurs auxquels l’utilisateur source s’est connecté aux environs de l’heure de l’activité.
...
1. Incluez les contrôleurs de domaine.
    ...
    - Recherchez les utilisateurs connectés aux environs de l’heure de l’activité, car ils peuvent également être compromis. Réinitialisez leurs mots de passe et activez l’authentification multifacteur (MFA) ou, si vous avez configuré les stratégies utilisateur à haut risque pertinentes dans Azure Active Directory Identity Protection, vous pouvez utiliser l'action [**Confirmer que l'utilisateur est compromis**](/cloud-app-security/accounts#governance-actions) dans le portail Cloud App Security.
1. Localisez l’ordinateur sur lequel l’utilisateur source était actif.
...
> [Tutoriel sur les alertes d’exfiltration](exfiltration-alerts.md)
