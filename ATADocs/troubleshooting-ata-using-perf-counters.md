...
2.  Arrêtez l’ensemble de collecteurs de données nommé : **Passerelle Microsoft ATA** ou **Centre Microsoft ATA**.
...
5.  Redémarrez l’ensemble de collecteurs de données nommé : **Passerelle Microsoft ATA** ou **Centre Microsoft ATA**.
...
Pour cela, ouvrez l’**Analyseur de performances**, puis ajoutez tous les compteurs à la passerelle ATA. Le nom de l’objet compteur de performances est : **Passerelle Microsoft ATA**.
...
> |Passerelle Microsoft ATA\Données de messages NetworkActivityTranslator # Taille de blocs|Quantité de trafic mise en file d’attente pour la traduction en activités réseau.|Doit être inférieur à la valeur maximale - 1 (valeur maximale par défaut : 100 000)|Vérifiez si un composant a atteint sa taille maximale et bloque les composants qui le précèdent jusqu’à NetworkListener. Reportez-vous à **Processus des composants ATA** ci-dessus.<br /><br />Vérifiez qu’il n’existe aucun problème avec le processeur ou la mémoire.|
> |Passerelle Microsoft ATA\Taille de blocs d’activité EntityResolver|Nombre d’activités réseau en attente de résolution.|Doit être inférieur à la valeur maximale - 1 (valeur maximale par défaut : 10 000)|Vérifiez si un composant a atteint sa taille maximale et bloque les composants qui le précèdent jusqu’à NetworkListener. Reportez-vous à **Processus des composants ATA** ci-dessus.<br /><br />Vérifiez qu’il n’existe aucun problème avec le processeur ou la mémoire.|
> |Passerelle Microsoft ATA\Taille de blocs de lots d’entités EntitySender|Quantité d’activités réseau en attente d’envoi vers le centre ATA.|Doit être inférieur à la valeur maximale - 1 (valeur maximale par défaut : 1 000 000)|Vérifiez si un composant a atteint sa taille maximale et bloque les composants qui le précèdent jusqu’à NetworkListener. Reportez-vous à **Processus des composants ATA** ci-dessus.<br /><br />Vérifiez qu’il n’existe aucun problème avec le processeur ou la mémoire.|
...
Pour cela, ouvrez l’**Analyseur de performances** et ajoutez tous les compteurs à la passerelle légère ATA. Le nom des objets compteurs de performances est : **Passerelle Microsoft ATA** et **Programme de mise à jour de la passerelle Microsoft ATA**.
...
Pour cela, ouvrez l’**Analyseur de performances** et ajoutez tous les compteurs au centre ATA. Le nom de l’objet compteur de performances est : **Centre Microsoft ATA**.
...
> |Compteur|Description|Seuil|Résolution des problèmes|
> |-----------|---------------|-------------|-------------------|
> |Microsoft ATA Center\EntityReceiver Entity Batch Block Size|Nombre de lots d’entités mis en file d’attente par le centre ATA.|Doit être inférieur à la valeur maximale - 1 (valeur maximale par défaut : 10 000)|Vérifiez si un composant a atteint sa taille maximale et bloque les composants qui le précèdent jusqu’à NetworkListener.  Reportez-vous à la section **Processus des composants ATA** précédente.<br /><br />Vérifiez qu’il n’existe aucun problème avec le processeur ou la mémoire.|
> |Microsoft ATA Center\NetworkActivityProcessor Network Activity Block Size|Nombre d’activités réseau en attente de traitement.|Doit être inférieur à la valeur maximale - 1 (valeur maximale par défaut : 50 000)|Vérifiez si un composant a atteint sa taille maximale et bloque les composants qui le précèdent jusqu’à NetworkListener. Reportez-vous à la section **Processus des composants ATA** précédente.<br /><br />Vérifiez qu’il n’existe aucun problème avec le processeur ou la mémoire.|
> |Microsoft ATA Center\EntityProfiler Network Activity Block Size|Nombre d’activités réseau en attente de profilage.|Doit être inférieur à la valeur maximale - 1 (valeur maximale par défaut : 100 000)|Vérifiez si un composant a atteint sa taille maximale et bloque les composants qui le précèdent jusqu’à NetworkListener. Reportez-vous à la section **Processus des composants ATA** précédente.<br /><br />Vérifiez qu’il n’existe aucun problème avec le processeur ou la mémoire.|
> |Centre Microsoft ATA\Base de données &#42; Taille de bloc|Nombre d’activités réseau d’un type spécifique en attente d’écriture dans la base de données.|Doit être inférieur à la valeur maximale - 1 (valeur maximale par défaut : 50 000)|Vérifiez si un composant a atteint sa taille maximale et bloque les composants qui le précèdent jusqu’à NetworkListener. Reportez-vous à la section **Processus des composants ATA** précédente.<br /><br />Vérifiez qu’il n’existe aucun problème avec le processeur ou la mémoire.|
> 
> 
> [!NOTE]
> -   Les valeurs temporelles des compteurs sont exprimées en millisecondes.
