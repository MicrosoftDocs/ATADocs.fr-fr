...
- NOUVEAU ! **Modification anormale de groupes sensibles**  
Dans le cadre de la phase de réaffectation de privilèges, des attaquants modifient des groupes avec des privilèges élevés pour accéder à des ressources sensibles. ATA détecte désormais la modification anormale d’un groupe élevé.
- NOUVEAU ! **Échecs d’authentification suspects** (comportement de force brute)  
Des attaquants tentent d’utiliser la force brute sur les informations d’identification pour compromettre des comptes. ATA déclenche désormais une alerte quand un comportement anormal d’authentification ayant échoué est détecté.

- **Tentative d’exécution à distance - Exécution WMI**  
Les attaquants peuvent tenter de contrôler votre réseau en exécutant du code à distance sur votre contrôleur de domaine. ATA a amélioré la détection de l’exécution à distance pour inclure la détection des méthodes WMI qui permettent d’exécuter du code à distance.

- Reconnaissance à l’aide de requêtes de service d’annuaire  
Cette détection a été améliorée pour être en mesure d’intercepter des requêtes sur une entité sensible unique et de réduire le nombre de faux positifs générés dans la version précédente. Si vous avez désactivé cette option dans la version 1.7, l’installation de la version 1.8 l’active désormais automatiquement.

- Activité de Golden Ticket Kerberos  
ATA 1,8 intègre une technique supplémentaire pour détecter les attaques golden ticket.
